...
bool Exercise4::createPipelineStateObject()
{
	ID3D12Device* device = app->getD3D12()->getDevice();
...
bool Exercise4::createSRVDescriptorHeap()
{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE textureCPUHandle = srvHeap->GetCPUDescriptorHandleForHeapStart();
	...
	device->CreateShaderResourceView(texture.Get(), &srvDesc, textureCPUHandle);
